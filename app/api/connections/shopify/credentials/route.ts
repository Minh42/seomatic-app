...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!connection || connection.connectionType !== 'shopify') {
      return NextResponse.json(
        { success: false, error: 'No Shopify connection found' },
        { status: 404 }
      );
    }
    ...
    // Return the decrypted token and store info
    return NextResponse.json({
      success: true,
      accessToken: credentials.apiKey,
      storeDomain: connection.connectionUrl,
      shopName: connection.cms?.cmsSiteId, // We store shop name in cmsSiteId
    });
  } catch (error) {
